...
- The plugin's last dropdown row shows the installed version (from the stamp, or
  a sibling `package.json` when run from a checkout — otherwise `vdev`) and links
  to the release list. **Not yet built:** the background check that tells you a
  newer release exists; that is the next iteration. Whatever it does, it must not
  install anything itself — the installer is interactive and a menu bar plugin is
  no place for a prompt.
